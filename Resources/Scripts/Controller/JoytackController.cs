...
namespace Game
{
    ...
    public class JoytackController : MonoBehaviour
    {
        ...
        private float distance, moveMaxDistance = 100, activationDistance = 1;
        //偏移矢量
        ...
        private void OnPointerDownDelegate(GameObject go)
        {
            if(controller == null)
            {
                controller = GlobalClient.GetPlayerController();
            }
            if (controller == null)
                return;
            //响应玩家控制类的对应函数
            if (controller.moveStart != null)
